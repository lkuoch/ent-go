...
import (
	...
	"errors"
	"fmt"
	"lkuoch/ent-todo/ent/__generated__/predicate"
...
func (tu *TodoUpdate) AddUserIDs(ids ...uuid.UUID) *TodoUpdate {
...
func (tu *TodoUpdate) RemoveUserIDs(ids ...uuid.UUID) *TodoUpdate {
...
func (tu *TodoUpdate) check() error {
	if v, ok := tu.mutation.Title(); ok {
		if err := todo.TitleValidator(v); err != nil {
			return &ValidationError{Name: "title", err: fmt.Errorf(`__generated__: validator failed for field "Todo.title": %w`, err)}
		}
	}
	if v, ok := tu.mutation.Priority(); ok {
		if err := todo.PriorityValidator(v); err != nil {
			return &ValidationError{Name: "priority", err: fmt.Errorf(`__generated__: validator failed for field "Todo.priority": %w`, err)}
		}
	}
	return nil
}
...
// AddUserIDs adds the "user" edge to the User entity by IDs.
func (tuo *TodoUpdateOne) AddUserIDs(ids ...uuid.UUID) *TodoUpdateOne {
...
// RemoveUserIDs removes the "user" edge to User entities by IDs.
func (tuo *TodoUpdateOne) RemoveUserIDs(ids ...uuid.UUID) *TodoUpdateOne {
...
func (tuo *TodoUpdateOne) sqlSave(ctx context.Context) (_node *Todo, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`__generated__: missing "Todo.id" for update`)}
	}
	...
	if fields := tuo.fields; len(fields) > 0 {
		...
		for _, f := range fields {
			if !todo.ValidColumn(f) {
				return nil, &ValidationError{Name: f, err: fmt.Errorf("__generated__: invalid field %q for query", f)}
			}
			if f != todo.FieldID {
