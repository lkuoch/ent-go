...
func (tq *TodoQuery) Exist(ctx context.Context) (bool, error) {
	...
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("__generated__: check existence: %w", err)
	default:
		return true, nil
	}
}
...
func (tq *TodoQuery) prepareQuery(ctx context.Context) error {
	for _, inter := range tq.inters {
		if inter == nil {
			return fmt.Errorf("__generated__: uninitialized interceptor (forgotten import __generated__/runtime?)")
		}
	...
	for _, f := range tq.ctx.Fields {
		if !todo.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("__generated__: invalid field %q for query", f)}
		}
	}
	if tq.path != nil {
